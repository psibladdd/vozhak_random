    ...
    return (//3
      <div style={{ position: 'fixed', top: 0,left:0,height:'100vh',width: '100vw'}}>
         ...
         <Canvas style={{ background: isNight ? "black" : 'skyblue',width:'100%',height:'100%' }}>
                     <StreetLamp position={[-0.9, 1.4, -1]} isNight={isNight} />
        <StreetLamp position={[0.6, 1.4, 1]} isNight={isNight} />
        <StreetLamp position={[0.5, 1.4, -0.8]} isNight={isNight} />
                <Cube rot={[0, Math.PI/4, 0]} size={[0.6, 1.5, 0.6]} color={"red"} pos={[0.4, 2.1, 0.4]}/>
                <Cube rot={[0, Math.PI/4, 0]} size={[3, 3, 3]} color={"green"} pos={[0,0,0]}/>
                <Road size={[3, 0.1, 0.4]} rot={[0, Math.PI/4, 0]} pos={[-0.42, 1.5, -0.42]}/>
                <Road size={[2.3, 0.1, 0.4]} rot={[0, -Math.PI/4, 0]} pos={[0.7, 1.5, -0.2] }/>
            <OrbitControls
                minDistance={5}
                maxDistance={50}
